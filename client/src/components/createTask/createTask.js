            ...
            <div className={style.innerWrapper}>
                ...
                <div className={`mt-2 ${style.row2}`}>
                    <div className={`${style.dlButton}`}>
                        {(!props.phaseStartDate || !props.phaseEndDate) && <input className={`${style.dlDate} inputNoBorder`} min={dateNow} type="date" value={deadlineDate} onChange={e => {setDeadlineDate(e.target.value)}}></input>}
                        {props.phaseStartDate && props.phaseEndDate && <input className={`${style.dlDate} inputNoBorder`} type="date" min={dateNow > props.phaseStartDate ? dateNow : props.phaseStartDate} max={props.phaseEndDate} value={deadlineDate} onChange={e => {setDeadlineDate(e.target.value)}}></input>}
                    </div>
                    <div className={`${style.dlButton}`}>

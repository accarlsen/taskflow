...
function EditTaskRM(props) {
            ...
            if (props.projectID && props.subtaskBool) {
                SetSubArchived({
                    ...
                    refetchQueries: [
                        ...
                        { query: getAuthorsTasks, skip: props.subtask.phaseID, variables: { authorID: userID.userID, orgID: orgID.orgID, archived: false, period: 0 } },
                        { query: getAuthorsTasks, skip: props.subtask.phaseID, variables: { authorID: userID.userID, orgID: orgID.orgID, archived: false, period: 1 } },
                        { query: getAuthorsTasks, skip: props.subtask.phaseID, variables: { authorID: userID.userID, orgID: orgID.orgID, archived: false, period: 2 } },
                        { query: getAuthorsTasks, skip: props.subtask.phaseID, variables: { authorID: userID.userID, orgID: orgID.orgID, archived: false, period: 3 } },
                        { query: getSubtasksOfParent, skip: props.subtask.phaseID, variables: { taskID: props.subtask.parentID } },
                ...
                SetArchived({
                    ...
                    refetchQueries: [
                        ...
                        { query: getAssignedTasks, skip: props.subtask.phaseID, variables: { assignedID: userID.userID, orgID: orgID.orgID, archived: false, period: 5 } },
                        { query: getAuthorsTasks, skip: props.subtask.phaseID, variables: { authorID: userID.userID, orgID: orgID.orgID, archived: true, period: 5 } },
            ...
            EditSubtask({
                ...
                refetchQueries:
                    [
                        ...
                        { query: getAuthorsTasks, variables: { authorID: userID.userID, orgID: orgID.orgID, archived: true, period: 5 } },
                        { query: getSubtasksOfParent, variables: { taskID: props.subtask.parentID } },
    ...
    if (!active) return ( //default - retracted
        <button className={`mb-4 ${style.buttonEdit}`} onClick={() => { setActive(true); setAssignedID(props.subtask.assignedID) }}>
    ...
    return ( //else - expanded
        <div className={`mb-2 ${style.wrapper}`} onKeyDown={handleKeyDown}>
                ...
                <div className={style.innerWrapper}>
                    ...
                    <div className={`mt-2 ${style.row2}`}>
                        <div className={`${style.dlButton}`}>
                            {!props.subtaskBool && <input className={`${style.dlDate} inputNoBorder`} type="date" value={deadlineDate} onChange={e => { setDeadlineDate(String(e.target.value)); }}></input>}
                            {props.subtaskBool && props.parentDeadline && <input className={`${style.dlDate} inputNoBorder`} type="date" min={dateNow} max={props.parentDeadline} value={deadlineDate} onChange={e => {
